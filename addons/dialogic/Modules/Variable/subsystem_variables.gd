...
func _set(property: StringName, value: Variant) -> bool:
	...
	if prop_str in var_storage:
		if typeof(var_storage[prop_str]) != TYPE_DICTIONARY:
			var_storage[prop_str] = value
			return true
...
func _get(property: StringName) -> Variant:
	...
	if prop_str in var_storage:
		if typeof(var_storage[prop_str]) == TYPE_DICTIONARY:
			return VariableFolder.new(var_storage[prop_str], prop_str, self)
		else:
			return DialogicUtil.logical_convert(var_storage[prop_str])
	return null
...
func merge_folder(new:Dictionary, defs:Dictionary) -> Dictionary:
	...
	for x in defs.keys(): # <--- Di sini tadi kurang kata "in"
		if not x in new:
			new[x] = defs[x]
	return new
#endregion
...
class VariableFolder:
	...
	func _get(property:StringName) -> Variant:
		var prop_str := str(property)
		if prop_str in data:
			if typeof(data[prop_str]) == TYPE_DICTIONARY:
				return VariableFolder.new(data[prop_str], path+"."+prop_str, outside)
			else:
				return DialogicUtil.logical_convert(data[prop_str])
		return null
	...
	func _set(property:StringName, value:Variant) -> bool:
		...
		if not value is VariableFolder:
			DialogicUtil._set_value_in_dictionary(path+"."+prop_str, outside.var_storage, value)
			return true
		return false
